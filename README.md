...
---
## Why TCD exists
Most production AI stacks still treat inference as a black-box API call. The application decides what to send, the model returns output, and whatever safety or governance exists is often split across SDK hooks, logs, ad hoc middleware, and post hoc review.
That leaves five recurring gaps:
...
This is the path that sees live inference traffic. It handles request identity, trusted transport context, authentication, edge security, edge rate limiting, and the transport adapters for HTTP and gRPC.
Its job is not to make the final governance decision. Its job is to turn an incoming request into a well-formed, budgeted, identity-bearing inference event.
### 2. Decision & Policy Plane
This is where detection, calibration, multivariate aggregation, policy binding, always-valid evidence processes, route planning, and security orchestration meet.
It produces the runtime decision surface:
- action
- reason
- enforcement mode
- route contract
...
This is where TCD becomes more than runtime middleware.
The governance and evidence plane includes the admin API, action agent, patch runtime, attestation, crypto envelope, schemas, signals, storage, ledger, audit log, chain auditor, and trust graph.
It governs control-plane changes and gives every important inference or mutation path a durable, verifiable afterlife.
---
## Request lifecycle
...
1. The HTTP or gRPC surface accepts a request and enforces hard limits on body size, headers, metadata, JSON depth, and endpoint budgets. It attaches request and event identifiers plus config and bundle fingerprints to the response path.
...
That explicitness matters. TCD is opinionated about governance, but it does not hide system boundaries behind vague enterprise language.

⸻

Repository map

The repository naturally groups into a few subsystems. Every file below is part of the runtime, control, or evidence story.
...
Canonical JSON, stable digests, message versions, bounded normalization, and stable event, decision, and route identifiers are treated as system primitives, not convenience features.
...
Untrusted input is always expected. Body size, JSON depth, cardinality, evidence node counts, string lengths, queue sizes, stream counts, chain windows, and patch segment sizes are bounded throughout the repository.
...
signals.py explicitly avoids becoming a generic event bus. service_grpc.py is a transport/control-plane adapter rather than a policy engine. storage.py is a governed persistence boundary rather than a casual helper layer. The repository is organized around inference governance, not SDK-bound application logic.
...
* The repository already defines a governed cryptographic control plane, but it should not be casually marketed as a fully production-complete PQ stack without specifying the exact backend and key-management environment in use.

That honesty is part of the design philosophy. Governance without accurate semantics is theater.
