...
This Quickstart exercises the full local governance receipt chain instead of only the minimal `/diagnose` request.

It validates, in one local run:
...
This Quickstart assumes the current `service_http.py` handles missing or null K7 hardware-root policy keys as an empty list. If uvicorn exits with:

```bash
TypeError: 'NoneType' object is not iterable
```

inside `_k7_policy_keys()`, fix that source bug first instead of bypassing the full-chain environment.
...
### 2. Clean old uvicorn / curl processes and port 8080

```bash
pgrep -af '[u]vicorn .*tcd\.service_http:create_app|[c]url .*127\.0\.0\.1:8080|[c]url .*localhost:8080' || true
pkill -f '[u]vicorn .*tcd\.service_http:create_app' || true
pkill -f '[c]url .*127\.0\.0\.1:8080|[c]url .*localhost:8080' || true
...
pkill -f '[u]vicorn .*tcd\.service_http:create_app' || true
pkill -f '[c]url .*127\.0\.0\.1:808[0-2]|[c]url .*localhost:808[0-2]' || true
```
...
### 3. Create a full-chain local runtime environment

```bash
PYTHON_BIN="$(command -v python3 || command -v python)"
TS="$(date +%Y%m%d_%H%M%S)"
RUN_DIR="/tmp/tcd-full-receipt-chain-$TS"
mkdir -p "$RUN_DIR"
...
### 4. Create the full-chain `/diagnose` payload
...
### 5. Compile and preflight the ASGI factory

```bash
python3 -m py_compile tcd/service_http.py tcd/verify.py tcd/storage.py tcd/attest.py tcd/security_router.py tcd/policies.py tcd/receipt_v2.py
...
### 6. Start uvicorn with the full-chain environment
...
echo "READY=$READY"
cat "$RUN_DIR/healthz.json" 2>/dev/null || true
echo
tail -n 120 "$RUN_DIR/uvicorn.log" 2>/dev/null || true
lsof -nP -iTCP:8080 -sTCP:LISTEN || true
```

A full-chain run should show runtime features such as:
...
### 8. Read readiness and runtime public state

```bash
curl -sS "$BASE_URL/readyz" > "$RUN_DIR/readyz.json"
curl -sS "$BASE_URL/runtime/public" > "$RUN_DIR/runtime.public.json"

python3 -m json.tool "$RUN_DIR/readyz.json" | sed -n '1,220p'
python3 -m json.tool "$RUN_DIR/runtime.public.json" | sed -n '1,260p'
```

### 9. Run full-chain `/diagnose`

```bash
TS="$(basename "$RUN_DIR" | sed 's/^tcd-full-receipt-chain-//')"
...
python3 -m json.tool "$RUN_DIR/diagnose.body.json" | sed -n '1,280p'
```

The response should include identifiers and artifacts such as:
...
### 10. Generate four verification requests
...
### 11. Verify receipt and evidence chain

```bash
curl -sS -X POST "$BASE_URL/verify" -H "Content-Type: application/json" --data-binary @"$RUN_DIR/verify.by_ref.json" > "$RUN_DIR/verify.by_ref.response.json"
curl -sS -X POST "$BASE_URL/verify" -H "Content-Type: application/json" --data-binary @"$RUN_DIR/verify.by_bundle.json" > "$RUN_DIR/verify.by_bundle.response.json"
curl -sS -X POST "$BASE_URL/verify" -H "Content-Type: application/json" --data-binary @"$RUN_DIR/verify.top_level.json" > "$RUN_DIR/verify.top_level.response.json"
curl -sS -X POST "$BASE_URL/verify" -H "Content-Type: application/json" --data-binary @"$RUN_DIR/verify.storage_window.json" > "$RUN_DIR/verify.storage_window.response.json"

python3 -m json.tool "$RUN_DIR/verify.by_ref.response.json" | sed -n '1,220p'
python3 -m json.tool "$RUN_DIR/verify.by_bundle.response.json" | sed -n '1,220p'
python3 -m json.tool "$RUN_DIR/verify.top_level.response.json" | sed -n '1,220p'
python3 -m json.tool "$RUN_DIR/verify.storage_window.response.json" | sed -n '1,260p'
```
...
Storage-window verification should also show:
...
### 12. Restart uvicorn and verify durable by-ref lookup
...
cat "$RUN_DIR/healthz.after_restart.json" 2>/dev/null || true
echo
...
python3 -m json.tool "$RUN_DIR/verify.by_ref.after_restart.response.json" | sed -n '1,240p'
```
...
### 13. Generate final full-chain summary
...
checks = {
    ...
    "diagnose_supply_chain_bound": bool((diag.get("build_id") or rv.get("build_id") or rp.get("build_id")) and (diag.get("image_digest") or rv.get("image_digest") or rp.get("image_digest"))),
...
summary = {
    ...
    "verify_reports": {
        "by_ref": by_ref.get("report"),
        "by_bundle": by_bundle.get("report"),
        "top_level": top.get("report"),
        "storage_window": window.get("report"),
        "after_restart_by_ref": after.get("report")
    }
}
...
### 14. Cleanup after the local run
