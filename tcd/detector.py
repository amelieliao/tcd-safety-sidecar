...
class Detector:
    ...
    def _build_static_hash_payload(self) -> Dict[str, Any]:
        """
        Build the static, secret-free payload used for config_hash / policy_digest.
        IMPORTANT:
        This payload is hashed through repo canonical_kv_hash() when available.
        Its KEY NAMES therefore must remain KV-safe and must not contain
        forbidden content tokens such as "raw", "body", "content", "input",
        or "output".
        We preserve the runtime semantics of EvidencePolicy.allow_raw_tenant,
        but we DO NOT expose that exact key name inside the hash payload.
        Instead we encode the same meaning under a KV-safe field:
            tenant_identity_mode = "clear_allowed" | "hashed_only"
        """
