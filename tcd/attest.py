...
def _extract_receipt_claims(
    *,
    ...
    claims: Dict[str, Any] = {
        ...
        "audit_ref": _coalesce_text(comp_obj.get("audit_ref"), meta_obj.get("audit_ref"), evidence_identity.get("audit_ref"), max_len=256),
        "receipt_ref": None,
        "ledger_ref": _coalesce_text(artifacts.get("ledger_ref"), max_len=256),
        "attestation_ref": None,
        "chain_namespace": _coalesce_text(meta_obj.get("chain_namespace"), max_len=128),
        "chain_id": _coalesce_text(meta_obj.get("chain_id"), max_len=256),
        "chain_seq": _coalesce_int(meta_obj.get("chain_seq")),
        "prev_head_hex": _coalesce_text(meta_obj.get("prev_head_hex"), max_len=256),
        ...
        "prepare_ref": _coalesce_text(artifacts.get("prepare_ref"), artifacts.get("ledger_prepare_ref"), max_len=256),
        "commit_ref": _coalesce_text(artifacts.get("commit_ref"), artifacts.get("ledger_commit_ref"), max_len=256),
        "outbox_ref": _coalesce_text(artifacts.get("outbox_ref"), max_len=256),
        ...
        "receipt_surface_kind": cfg.default_surface_kind,
        "receipt_delivery_state": cfg.default_delivery_state,
        "evidence_durable": bool(cfg.default_evidence_durable),
        "evidence_storage_ready": bool(cfg.default_evidence_storage_ready),
        "ledger_stage_defaulted": ledger_stage == cfg.default_ledger_stage,
    }

    produced_by = _safe_tag_list(
        ["tcd.attest", cfg.attestor_id, comp_obj.get("kind"), meta_obj.get("event_type")],
        max_items=16,
        max_len=64,
    )
...
def _normalize_issue_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "attestation_ref" not in out:
        out["attestation_ref"] = out.get("receipt_ref")
    if "attestation_id" not in out:
        out["attestation_id"] = out.get("receipt_ref")
