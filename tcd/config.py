...
# IMPORTANT:
# Pydantic v2 forbids defining both `model_config = ...` and inner `class Config`
# on the same model. Detect v2 explicitly and keep those branches mutually exclusive.
_PYDANTIC_V2 = bool(ConfigDict is not None and hasattr(BaseModel, "model_validate"))
...
def _parse_config_mapping(raw: bytes, fmt: str) -> Tuple[Dict[str, Any], Optional[str]]:
    ...
    return doc, None
# ---------------------------------------------------------------------------
# Settings model (v2-safe, with v1 fallback)
# ---------------------------------------------------------------------------
class Settings(BaseModel):
    ...
    # IMPORTANT:
    # Pydantic v2 cannot have `model_config` and inner `class Config` together.
    # Keep them mutually exclusive at class-definition time.
    ...
    def field_names(cls) -> FrozenSet[str]:
        if _PYDANTIC_V2:
            ...
            return frozenset()
        ff = getattr(cls, "__fields__", None)  # pragma: no cover - pydantic v1 fallback
...
def _normalize_settings_dict(raw: Mapping[str, Any], *, base: Settings, gov: Governance) -> Tuple[Dict[str, Any], List[str], List[str]]:
    ...
    def _clamp_float_local(name: str, lo: float, hi: float) -> None:
    ...
    _clamp_float_local("alpha", _MIN_ALPHA, _MAX_ALPHA)
    _clamp_float_local("slo_latency_ms", 1.0, 120_000.0)
    _clamp_float_local("http_rate_capacity", 0.0, _MAX_RATE)
    _clamp_float_local("http_rate_refill_per_s", 0.0, _MAX_RATE)
    _clamp_float_local("token_cost_divisor_default", _MIN_ALPHA, _MAX_TOKEN_COST_DIV)  # reuse min guard
    _clamp_float_local("eprocess_alpha_default", _MIN_ALPHA, _MAX_ALPHA)
    _clamp_float_local("eprocess_alpha_max_delta_per_reload", 1e-12, 0.5)
    _clamp_float_local("eprocess_min_wealth", 0.0, 1e12)
    ...
    def refresh(self) -> Settings:
        ...
        with self._lock:
            ...
            if not er.ok:
                ...
                return old
            # No-op detection by effective hash
